...
function ExploreMenu( ) {
  ...
  return (
    <div className="ExploreMenu w-[89%] mx-auto">
      ...
      <p className="Explore-menu-text my-3 max-w-[60%]">
      ...
      <Slider {...settings}>
        {menu_list.map((item, index) => (
          <div  className="Explore_menu-list-item cursor-pointer  items-center transition-transform transform hover:scale-105 p-4" key={index}>
            <img  className= "h-auto" src={item.menu_image} alt={item.menu_name}  />
            <p className="text-center mt-2 items-center">{item.menu_name}</p>
          </div>
        ))}
      </Slider>
